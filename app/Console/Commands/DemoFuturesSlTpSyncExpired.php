...
class DemoFuturesSlTpSyncExpired extends Command
{
    ...
    private function syncForUserExchange(int $userId, $userExchange): void
    {
        ...
        try {
            ...
            foreach ($filledOrdersWithSl as $order) {
                try {
                    $this->syncStopLossForOrder($exchangeService, $order, $positions, $conditionalOrders, $symbol, $userExchange);
                } catch (\Throwable $e) {
    ...
    private function syncStopLossForOrder(ExchangeApiServiceInterface $exchangeService, $order, array $positions, array $conditionalOrders, string $symbol, $userExchange): void
    {
        ...
        try {
            ...
            $apiResult = $exchangeService->createOrder($orderParams);
            
            // Exchange-specific success detection
            $success = false;
            $exchangeName = strtolower($userExchange->exchange_name ?? '');
            
            switch ($exchangeName) {
                case 'bybit':
                    $success = isset($apiResult['retCode']) && $apiResult['retCode'] === 0;
                    break;
                case 'binance':
                case 'bingx':
                    // Binance and BingX throw exceptions on errors, so if we reach here, it's successful
                    $success = true;
                    break;
                default:
            ...
            if ($success) {
                ...
                $this->warn("    Failed to create demo stop loss for order {$order->order_id}: " . json_encode($apiResult));
            }
