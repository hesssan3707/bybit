...
class DemoFuturesOrchestrator extends Command
{
            ...
            if ($dbOrder->cancel_price) {
                try {
                    ...
                    if ($shouldCancel) {
                        ...
                        } catch (Exception $e) { $this->warn("    خطا در لغو سفارش دمو (قیمت بسته شدن) {$dbOrder->order_id}: " . $e->getMessage()); }
                        continue;
                    }
                } catch (Exception $e) {
    ...
    private function syncLifecycleForExchange($exchangeService, User $user, UserExchange $ue)
    {
        ...
        } else { $this->info("سفارش دمو یافت نشد"); }
    ...
    private function syncPnlRecords($exchangeService, UserExchange $ue)
    {
        ...
        } catch (Exception $e) { $this->warn("خطا در همگام‌سازی سوابق PnL دمو: " . $e->getMessage()); }
    }
    ...
    private function verifyClosedTradesSynchronization($exchangeService, UserExchange $ue)
    {
        ...
        } catch (Exception $e) { $this->warn("خطا در تأیید همگام‌سازی معاملات بسته دمو: " . $e->getMessage()); }
    }
